...
To add a new dependency, navigate to the project's directory and run `rush add -p "<packagename@^version>" [--dev]`. This will update the project's package.json and then automatically run `rush update` to install the package into the project's node_modules directory. Do not use `npm install [--save | --save-dev]`.
...
If you manually edit the package.json, make sure to run `rush update` afterwards to update the project's node_modules directory.
